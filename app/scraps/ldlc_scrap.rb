class LdlcScrap
  ...
  DOMAIN = 'http://www.ldlc.com'.freeze
  URL = "#{DOMAIN}/informatique/ordinateur-portable/pc-portable/c4265/".freeze
  ...
  def self.scrap_pc(url)
    ...
    hash_memory[:memory_size] = extract_from_hash(hash_main, 'Taille de la mémoire').gsub(/go/i,"").strip.to_i rescue nil
    ...
    hash_screen[:screen_resolution] = extract_from_hash(hash_main, 'Résolution Max').gsub(/pixels/i, "").strip rescue nil
